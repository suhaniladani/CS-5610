defmodule TasktrackerWeb.ManageController do
  ...
  def index(conn, _params) do
    manages = Tasklist.list_manages()
    render(conn, "index.json", manages: manages)
  end

  def create(conn, %{"manage" => manage_params}) do
    with {:ok, %Manage{} = manage} <- Tasklist.create_manage(manage_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "manage" => manage_params}) do
    ...
    with {:ok, %Manage{} = manage} <- Tasklist.update_manage(manage, manage_params) do
      render(conn, "show.json", manage: manage)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    manage = Tasklist.get_manage!(id)
    with {:ok, %Manage{}} <- Tasklist.delete_manage(manage) do
